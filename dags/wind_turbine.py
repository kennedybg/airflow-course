...
default_args = {
    ...
    'email': ['kennedybg.droid@gmail.com'],
...
dag = DAG('wind_turbine', description='Dados da Turbina',
          schedule_interval=None, start_date=datetime(2023, 3, 5),
          catchup=False, default_args=default_args, default_view='graph',
          doc_md="## Dag para registrar dados de turbina eólica", tags=['final'])


group_check_temp = TaskGroup("group_check_temp", dag=dag)
group_databse = TaskGroup("group_database", dag=dag)
...
def process_file(**kwarg):
    with open(Variable.get('path_file')) as f:
        data = json.load(f)
        kwarg['ti'].xcom_push(key='idetemp', value=data['idtemp'])
...
create_table = PostgresOperator(task_id="create_table",
                        ...
                        sensors (idtemp varchar, powerfactor varchar, hydraulicpressure varchar, temperature varchar)''',
                                task_group=group_databse,
                                dag=dag)
...
insert_data = PostgresOperator(task_id="insert_data",
                               ...
                               sql='''INSERT INTO sensors (idtemp, powerfactor, hydraulicpressure, temperature, timestamp) VALUES (%s, %s, %s, %s, %s);''',
                               task_group=group_databse,
                               dag=dag
                               )
